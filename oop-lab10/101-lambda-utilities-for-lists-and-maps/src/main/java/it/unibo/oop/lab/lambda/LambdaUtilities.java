...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        ...
        list.forEach(i -> optList.add(Optional.of(i).filter(pre)));
        return optList;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
         ...
         */
        final Map<R, Set<T>> grouppedMap = new HashMap<>();
        // list.forEach(i -> grouppedMap.merge(op.apply(i), new HashSet<>(Set.of(i)), new BiFunction<Set<T>,Set<T>,Set<T>>() {
        ...
        //     } 
        // }));
        list.forEach(i -> grouppedMap.merge(op.apply(i), Set.of(i), Sets::union));
        return grouppedMap;
    }
    ...
    public static void main(final String[] args) {
        ...
        .mapToObj(i -> Integer.valueOf(i)).collect(Collectors.toList());  // NOPMD suppressed as it is a false positive
